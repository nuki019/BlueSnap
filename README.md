...
> 把创造的自由还给用户 —— 一句话，把校园里的临时需求变成手机里可运行、可保存、可分享的轻工具

2026 中国大学生计算机设计大赛 AIGC 创新赛道参赛作品

## 项目简介

蓝心快搭 (BlueSnap) 是一个 Android 应用，面向 18-24 岁大学生和准职场青年。用户通过自然语言描述校园里的临时工具需求，AI 自动生成完整的单页 HTML 应用并在 WebView 沙箱中实时预览。无需编程知识，无需安装额外应用，即说即用。

**核心能力：**
- 自然语言理解：支持中文描述活动、课程、小组、求职和生活管理需求，AI 自动解析意图
...
- 离线导出与分享：生成的 HTML 可保存到本地，也可进入“分享成果”页通过系统分享给同学
...
└── ui/
    ...
    └── screens/
        ...
        └── ShareScreen.kt          # 结果分发页
```

## 演示流程

1. 打开应用 → 首页展示校园场景卡（活动筹备、课程协作、求职实习、生活管理）
2. 点击场景卡或手动输入 → "给迎新活动做一个报名、分工和预算管理工具"
3. AI 流式回复 → 实时显示思考过程
4. 查看方案 → 功能清单可自定义开关
5. 确认生成 → AI 生成完整 HTML 应用
6. 预览运行 → WebView 内直接操作，支持数据持久化
7. 进入分享成果页 → 导出 HTML 或通过系统分享给同学
...
- APK 和源码扫描不得包含真实 Key。
- PPT 使用真实截图，禁用流程图；首页、预览页、分享成果页都可作为截图重点。
- 视频为 MP4，≤3分钟，≥1080P。
