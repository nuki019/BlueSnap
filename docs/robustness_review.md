...
| 分享 HTML | 可分享 cache 文件 | 中 | FileProvider + cache-path | 接收方能否打开取决于系统/应用；保留导出作为稳妥路径 |
| 离线导出 HTML | 可用系统文件创建器保存 | 低 | SAF `CreateDocument("text/html")` | 演示时导出到 Downloads，证明离线可留存 |
