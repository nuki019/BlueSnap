...
蓝心快搭不是“又一个代码生成器”，而是面向 18-24 岁大学生和准职场青年的校园效率轻工具生成器。它把“我现在需要一个小工具”的表达，直接变成手机里能运行、能保存、能分享的轻应用。
...
- 扩展表达：同一套轻工具生成闭环可以继续覆盖教师助教、内容创作者、小微商家，但复赛主线不展开。

答辩时不要把用户讲得过散。结构应为：
1. 先讲主线：校园里的临时工具需求高频、碎片、强个性化。
2. 再落到黄金样例：校园活动筹备助手。
3. 最后回到可扩展：同一闭环可复用到课程协作、求职实习和生活管理。

## 2. 一句话定义

用户用自然语言说出校园里的临时需求，蓝心快搭先生成可确认的方案，再生成单 HTML 轻应用，并在手机 WebView 沙箱中即时运行。生成结果会进入历史记录，支持再次打开、删除，并可在“分享成果”页离线导出和系统分享。
...
- “蓝心快搭选择更小但更确定的切口：校园效率、手机端、单 HTML、WebView 沙箱、离线导出。”
- “首页不是流程图式入口，而是手机工具台：蓝色品牌区、四张校园场景卡、底部创造/历史入口，让用户先看到能做什么。”
- “评委看到的不是概念图，而是从输入到运行的完整链路。”
- “它解决的是校园长尾轻工具：功能小、周期短、个人化强，不值得下载大 App，也不值得开电脑开发。”
...
答：它们更偏云端 Web/full-stack 工程生成，适合部署成网站或完整项目。蓝心快搭聚焦校园效率轻应用：自然语言生成单 HTML，在 Android WebView 沙箱里立即运行，并能进入分享成果页离线导出和系统分享。我们的目标不是替代云 IDE，而是让大学生和准职场青年在手机上快速得到一个“刚好够用”的小工具。
...
**问：为什么用户愿意用？**  
答：因为校园里的很多需求是即时、个人化、低频但刚需的。比如活动筹备、课程任务、小组分工、求职投递、生活预算，以及课堂笔记/项目复盘用的快捷思维导图。现成 App 往往太重或不贴合，低代码工具又需要学习。蓝心快搭把这个中间地带变成“说一句话即可试用”。

## 6. PPT 叙事顺序

1. 封面：把创造的自由还给用户。
2. 青年用户洞察：大学生和准职场青年需要大量临时小工具。
3. 校园长尾痛点：活动、课程、小组、求职、生活需求碎片化。
4. 首页真实截图：蓝色品牌区、四类校园场景卡、浮层底部导航。
5. 黄金演示截图：活动筹备助手的一句话输入。
6. 方案确认截图：AI 不是直接替用户决定，用户可控。
7. 预览运行截图：WebView 里真实可操作。
8. 分享成果页截图：从“生成”走向“可留存、可流转”。
9. 竞品对比与安全：不做云 IDE，做手机端轻应用闭环。
10. 总结：让校园里的每个临时需求，都能变成手机里的小工具。

备用演示：输入“帮我制作一个快捷思维导图应用”，展示节点编辑、层级调整、复制大纲和本地保存，证明产品不是单一活动筹备模板。

## ????????

- ?????????????????? HTML ????App ? WebView ????????????HTML ??/?????
- ????????????????? Mock ????????????????????????????
- ??????????????TTS?ASR?Function calling??? 3B ????? App ??? `image_prompt` / `audio_prompt` ?????????????????????
- ??????? HTML ????? CDN??? API?????????WebView ??????????DeepSeek ??????????????? APK?
